...
def run_isotimemeans(df,col_var):
    print(f'{73 * "-"}\nStarting analysis of variable means over time\n{73 * "-"}')
...
def run_heatmaps(df,hue_var):
    ...
    df['down']=-df['dvz'] #df['down']=-df['dvz_s']
...
def do_stats(df,thr_reg_var,time_var):
    ...
    dfc.save_table(df_desc,'Descriptive stats'+'_'+ test_var+ thr_reg_var)
    dfc.save_table(df_tests,'Statistical tests'+'_'+ test_var+ thr_reg_var)
